...
const maxCharacters = (width: number, fontSize: number) => Math.max(1, Math.floor((width - 8) / (fontSize * 0.52)));
...
const textCommand = (text: string, x: number, y: number, size: number, bold = false) =>
    `BT /${bold ? 'F2' : 'F1'} ${size} Tf ${x.toFixed(2)} ${y.toFixed(2)} Td (${text}) Tj ET`;
