 ...
 */
export function countCompletedJuz(logs: { surah_name: string | null; start_v: number | null; end_v: number | null }[]): number {
    const ranges: { start: number; end: number }[] = [];

    for (const log of logs) {
        if (!log.surah_name || !log.start_v || !log.end_v) continue;
        const sid = getSurahId(log.surah_name);
        if (!sid) continue;
        const s = toGlobal(sid, log.start_v);
        const e = toGlobal(sid, log.end_v);
        if (s >= 0 && e >= s) ranges.push({ start: s, end: e });
    }
