 ...
 */
export const copyPreviousYearPlacements = async (req: Request, res: Response) => {
    const client = await db.getClient();
    try {
        const targetAcademicYearId = req.params.id;
        const requestedSourceYearId = req.body?.source_academic_year_id;
        const targetRes = await client.query(
            'SELECT id, name, start_date FROM academic_years WHERE id = $1',
            [targetAcademicYearId],
        );
        if (!targetRes.rows.length) return res.status(404).json({ success: false, error: 'Academic year not found.' });
        ...
        const sourceRes = requestedSourceYearId
            ...
            : await client.query(
                ...
                [targetAcademicYearId, targetRes.rows[0].start_date],
            );
        if (!sourceRes.rows.length) return res.status(400).json({ success: false, error: 'No earlier academic year is available to copy.' });

        const sourceAcademicYearId = sourceRes.rows[0].id;
        await client.query('BEGIN');
        const copiedDivisions = await client.query(
        ...
        const copiedPlacements = await client.query(
            `INSERT INTO academic_student_placements (academic_year_id, student_id, standard, division, status, created_at, updated_at)
        ...
        const copiedSnapshots = await client.query(
