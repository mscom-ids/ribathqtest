    ...
    try {
        ...
        if (id) {
            result = await db.query(
                `UPDATE academic_years SET name=$1, start_date=$2, end_date=$3, is_current=$4, is_locked=$5 WHERE id=$6 RETURNING *`,
                [name, start_date, end_date, is_current || false, is_locked || false, id]
            );
        } else {
            result = await db.query(
                `INSERT INTO academic_years (name, start_date, end_date, is_current, is_locked) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
                [name, start_date, end_date, is_current || false, is_locked || false]
            );
        }
