...
function missingHistoryLayerMessage(error: any) {
...
type YearStartPlan = {
    ...
    school_rules?: PromotionRule[];
    madrasa_rules?: PromotionRule[];
    excluded_school_student_ids?: string[];
    excluded_madrasa_student_ids?: string[];
    excluded_hifz_student_ids?: string[];
    hifz?: {
        carry_mentor?: boolean;
        carry_group?: boolean;
        mentor_id?: string | null;
        hifz_group_class_id?: string | null;
    };
};
...
function nextStandard(value: unknown) {
    const text = normalizeText(value);
    if (!text) return '';
...
function buildPromotionMap(rules: PromotionRule[] | undefined, defaults: Array<{ standard: string; section: string | null }>) {
    const map = new Map<string, { to_standard: string; to_section: string | null }>();
...
function ruleFor(map: Map<string, { to_standard: string; to_section: string | null }>, standard: string, section: string | null) {
    return map.get(`${standard}|||${section || ''}`)
        || map.get(`${standard}|||`)
        || { to_standard: nextStandard(standard), to_section: section || null };
}

function idSet(values: unknown) {
    return new Set(
...
async function buildYearStartPreview(plan: YearStartPlan) {
    ...
    const [yearRes, targetSettings, schoolDefaultsRes, madrasaDefaultsRes] = await Promise.all([
        ...
        db.query(
            `SELECT COALESCE(ays.year_locked, ay.is_locked, false) AS year_locked
    ...
    if (yearRes.rows.length < 2) throw new Error('Source or target academic year not found');
    if (targetSettings.rows[0]?.year_locked) throw new Error('Target academic year is locked');
    ...
    const [schoolSourceRes, madrasaSourceRes, hifzRes, existingTargetRes] = await Promise.all([
        db.query(
            `SELECT se.student_id, s.name, se.school_standard, se.school_section
             ...
             ORDER BY se.school_standard, se.school_section, s.name`,
            [source_academic_year_id]
        ),
        db.query(
            `WITH history_rows AS (
             ...
             ORDER BY madrasa_standard, madrasa_section, name`,
            [source_academic_year_id]
        ),
        db.query(
            `SELECT hp.student_id, s.name, hp.mentor_id, st.name AS mentor_name,
    ...
    const all_school_students = schoolSourceRes.rows.map((row: any) => {
    ...
    const all_madrasa_students = madrasaSourceRes.rows.map((row: any) => {
    ...
    const all_hifz_students = hifzRes.rows.map((row: any) => ({
        student_id: row.student_id,
        name: row.name,
    ...
    const school_students = all_school_students.filter((row: any) => !excludedSchoolIds.has(row.student_id));
    const madrasa_students = all_madrasa_students.filter((row: any) => !excludedMadrasaIds.has(row.student_id));
    const hifz_students = all_hifz_students.filter((row: any) => !excludedHifzIds.has(row.student_id));

    const touched = new Set<string>();
    school_students.forEach((item: any) => touched.add(item.student_id));
    madrasa_students.forEach((item: any) => touched.add(item.student_id));
    hifz_students.forEach((item: any) => touched.add(item.student_id));
...
export const getAcademicYearsWithSettings = async (_req: Request, res: Response) => {
    try {
...
export const upsertAcademicYearSettings = async (req: Request, res: Response) => {
    try {
        ...
        const result = await db.query(
            `INSERT INTO academic_year_settings (
                academic_year_id,
                school_fee_plan_id,
                madrasa_fee_plan_id,
                promotion_completed,
...
export const previewYearStart = async (req: Request, res: Response) => {
    try {
...
export const commitYearStart = async (req: Request, res: Response) => {
    const client = await db.getClient();
    try {
        const plan: YearStartPlan = req.body;
        ...
        const schoolBatch = preview.school_students.length > 0
            ? await client.query(
                `INSERT INTO promotion_batches (from_academic_year_id, to_academic_year_id, track_type, created_by)
                 VALUES ($1, $2, 'school', $3)
                 RETURNING id`,
                [sourceYearId, targetYearId, userId]
            )
            : { rows: [] };
        ...
        for (const row of preview.school_students as any[]) {
            const result = await client.query(
                ...
                [row.student_id, targetYearId, row.to_standard, row.to_section || null]
            );
            if (result.rows[0]?.inserted) schoolCreated++;
            if (schoolBatch.rows[0]?.id) {
                await client.query(
                    `INSERT INTO promotion_logs (
                     ...
                     VALUES ($1, $2, 'school', $3, $4, $5, $6)`,
                    [schoolBatch.rows[0].id, row.student_id, row.from_standard, row.to_standard, row.from_section || null, row.to_section || null]
                );
                schoolLogs++;
            }
        ...
        for (const row of preview.madrasa_students as any[]) {
            const result = await client.query(
                `INSERT INTO student_madrasa_enrollments (
            ...
            if (result.rows[0]?.inserted) madrasaCreated++;
            if (madrasaBatch.rows[0]?.id) {
                await client.query(
                    `INSERT INTO promotion_logs (
                     ...
                     VALUES ($1, $2, 'madrasa', $3, $4, $5, $6)`,
                    [madrasaBatch.rows[0].id, row.student_id, row.from_standard, row.to_standard, row.from_section || null, row.to_section || null]
                );
                madrasaLogs++;
            }
            merged.set(row.student_id, {
        ...
        for (const row of Array.from(merged.values())) {
            ...
            if (result.rows[0]?.inserted) snapshotsCreated++;
        }

        const reportRes = await client.query(
            `INSERT INTO academic_year_migration_reports (
                migration_name,
                academic_year_id,
                total_students,
        ...
        await client.query(
            `INSERT INTO academic_year_settings (academic_year_id, promotion_completed, updated_at)
             VALUES ($1, true, now())
             ON CONFLICT (academic_year_id) DO UPDATE SET
...
export const getStudentAcademicHistory = async (req: Request, res: Response) => {
    try {
        const { studentId } = req.params;
        const [studentRes, schoolRes, madrasaRes, hifzRes, snapshotRes] = await Promise.all([
            db.query(
                `SELECT adm_no, name, status, standard, batch_year, hifz_mentor_id, school_mentor_id, madrasa_mentor_id
                 FROM students
                 WHERE adm_no = $1`,
                [studentId]
            ),
            db.query(
                `SELECT se.*, ay.name AS academic_year_name, ay.start_date, ay.end_date
                 FROM student_school_enrollments se
            ...
            db.query(
                `SELECT me.*, ay.name AS academic_year_name, ay.start_date, ay.end_date
                 FROM student_madrasa_enrollments me
            ...
            db.query(
                `SELECT hp.*, st.name AS mentor_name
                 FROM student_hifz_profiles hp
                 LEFT JOIN staff st ON st.id = hp.mentor_id
            ...
            db.query(
                `SELECT sys.*, ay.name AS academic_year_name, ay.start_date, ay.end_date
...
export const getYearSnapshots = async (req: Request, res: Response) => {
    try {
        const context = await getAcademicYearContext(db, req.query.academic_year_id);
        if (!context.academicYearId) {
        ...
        const params: any[] = [context.academicYearId];
        ...
        const [rowsRes, countRes] = await Promise.all([
            db.query(
                `SELECT sys.*, s.name, s.photo_url, s.batch_year
                 FROM student_year_snapshots sys
...
export const getMigrationReports = async (_req: Request, res: Response) => {
    try {
        const result = await db.query(
            `SELECT *
...
export const getHistoryHealth = async (req: Request, res: Response) => {
    try {
        const context = await getAcademicYearContext(db, req.query.academic_year_id);
        const result = await db.query(
            `WITH active_students AS (
             ...
             SELECT
                ...
                (SELECT COUNT(*)::integer
                 ...
                 )) AS active_students_missing_snapshot`,
            [context.academicYearId]
        );
