...
export const getCalendarRange = async (req: Request, res: Response) => {
    try {
...
export const createSession = async (req: Request, res: Response) => {
    try {
        ...
        const result = await db.query(
            `INSERT INTO academic_sessions (name, type, start_time, end_time, days_of_week, is_active, standards)
...
export const updateSession = async (req: Request, res: Response) => {
    try {
        ...
        const { name, type, start_time, end_time, days_of_week, is_active, standards } = req.body;
        const result = await db.query(
            `UPDATE academic_sessions SET name=$1, type=$2, start_time=$3, end_time=$4, days_of_week=$5, is_active=$6, standards=$7
...
export const upsertCalendarPolicy = async (req: Request, res: Response) => {
    try {
        const p = req.body;
        const result = await db.query(
            `INSERT INTO academic_calendar (date, is_holiday, description, day_mode, effective_day_of_week,
             ...
             RETURNING *`,
            [p.date, p.is_holiday, p.description || null, p.day_mode || 'Normal', p.effective_day_of_week || null,
...
export const deleteCalendarPolicy = async (req: Request, res: Response) => {
    try {
        const { date } = req.params;
...
export const bulkUpsertCalendarPolicies = async (req: Request, res: Response) => {
...
export const generateCalendarEntries = async (req: Request, res: Response) => {
...
export const getDisciplinaryRecords = async (req: Request, res: Response) => {
    try {
...
export const createDisciplinaryRecord = async (req: Request, res: Response) => {
    try {
        ...
        const user = (req as any).user;
        const result = await db.query(
            `INSERT INTO disciplinary_records (student_id, title, description, severity, points, action_date, status, recorded_by)
             VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
            [student_id, title, description || null, severity || 'Low', points || 0, action_date, status || 'Pending', user?.id || null]
        );
...
export const deleteDisciplinaryRecord = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        await db.query('DELETE FROM disciplinary_records WHERE id = $1', [id]);
        res.json({ success: true });
    } catch (err) {
