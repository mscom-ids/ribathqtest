        ...
        const [activeLeaveResult, logsResult, attResult, outgoingDelegationsRes, lastHifzResult] = await Promise.all([
            ...
            safeRows('today hifz logs lookup', db.query(
                `SELECT student_id, mode, start_page, end_page, juz_portion, entry_date
                 FROM hifz_logs WHERE student_id = ANY($1) AND entry_date = $2 AND deleted_at IS NULL`,
...
export const getMyLeaves = async (req: Request, res: Response) => {
    try {
        ...
        // Mentors can view and close leaves only for their assigned students.
        // This matches the authorization enforced by recordReturn.
        const assignedLeavesRes = await db.query(
        ...
        const allRows = assignedLeavesRes.rows;
