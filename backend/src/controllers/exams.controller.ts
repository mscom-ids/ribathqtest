...
export const createExam = async (req: Request, res: Response) => {
    try {
        ...
        const result = await db.query(
            `INSERT INTO exams (title, department, type, start_date, end_date, is_active)
             VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
            [title, department, type || department, start_date, end_date || null, is_active ?? true]
        );
...
export const getExamDetails = async (req: Request, res: Response) => {
    try {
        ...
        const subjectsResult = await db.query('SELECT * FROM exam_subjects WHERE exam_id = $1 ORDER BY created_at', [id]);
...
export const deleteSubject = async (req: Request, res: Response) => {
    try {
        ...
        await db.query('DELETE FROM exam_subjects WHERE id = $1', [subject_id]);
...
export const upsertExamMarks = async (req: Request, res: Response) => {
    try {
        const { id } = req.params; // exam_id
