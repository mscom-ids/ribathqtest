...
export const generateMonthlyFees = async (req: Request, res: Response) => {
    try {
        const result = await db.query(`SELECT adm_no, custom_monthly_fee FROM students WHERE status = 'active'`);
        ...
        const feePlanResult = await db.query(
            `SELECT amount FROM fee_plans WHERE effective_from <= $1 ORDER BY effective_from DESC LIMIT 1`,
            [today]
        );
        ...
        const existingFeesResult = await db.query(
            `SELECT student_id FROM monthly_fees WHERE month = $1`,
            [monthFirstDay]
        );
        ...
        await db.query(
            `INSERT INTO monthly_fees (student_id, month, base_fee, final_fee, balance, status) VALUES ${insertValues}`,
            flatValues
        );
...
export const deleteMonthlyFeesForMonth = async (req: Request, res: Response) => {
    try {
        ...
        const result = await db.query(
            `DELETE FROM monthly_fees WHERE month = $1 AND status IN ('pending', 'partial') RETURNING id`,
            [targetMonth]
        );
