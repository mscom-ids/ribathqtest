...
function subjectGroupKey(options: Options, _schedule: any, subjectName: string) {
    return [options.academicYearId, options.department, normalizeSubjectName(subjectName)].join('::');
}

function sortSubjectGroups(a: Pick<SubjectGroup, 'name'>, b: Pick<SubjectGroup, 'name'>) {
...
export const getManagementAttendanceReport = async (req: Request, res: Response) => {
    try {
        ...
        const result = await cachedResult(makeCacheKey('reports:management-attendance:v6', cacheParts(options)), 120000, async () => {
            const scheduleSql = `
            ...
            for (const schedule of schedulesResult.rows) {
                ...
                if (schedule.subject_id) subject.sourceIds.add(String(schedule.subject_id));
                subject.sourceIds.add(String(schedule.id));
            }

            const activeSubjectGroups = Array.from(subjectsMap.values())
                .filter(subject => students.some((student: any) =>
                    subject.schedules.some((schedule: any) => studentMatchesSchedule(student, schedule))
                ))
            ...
            if (activeSubjectGroups.length > 1) {
                ...
                for (const subject of activeSubjectGroups) {
                    for (const sourceId of subject.sourceIds) allSourceIds.add(sourceId);
                }
            ...
            if (selectedSubId && subjectsMap.has(selectedSubId)) {
                ...
                const subjectDatesSet = new Set<string>();
                ...
                if (subjectDates.length > 0) {
                    ...
                    const rows = students.filter((student: any) =>
                        subject.schedules.some((schedule: any) => studentMatchesSchedule(student, schedule))
                    ).map((student: any) => {
                        ...
                        for (const day of subjectDates) {
                            ...
                            const studentSchedules = subject.schedules.filter((schedule: any) =>
                                scheduleApplies(schedule, day) && studentMatchesSchedule(student, schedule)
                            );
                            ...
                            for (const totalCode of totalCodes) {
                                if (totalCode === 'C') {
                                    ...
                                    totals.total += 1;
                                    if (totalCode === 'N') totals.pending += 1;
                                    if (totalCode === 'P') { totals.present += 1; summary.present += 1; }
                                    if (totalCode === 'A') { totals.absent += 1; summary.absent += 1; }
                                    if (totalCode === 'L') { totals.leave += 1; summary.leave += 1; }
                                }
                            }
                        }
