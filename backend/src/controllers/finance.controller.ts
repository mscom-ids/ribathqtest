...
export const getStudentLedger = async (req: Request, res: Response) => {
  try {
    ...
    `;
    const feeRecords = await db.query(feeRecordsQuery, [student_id]);
    ...
    `;
    const payments = await db.query(paymentsQuery, [student_id]);
  ...
  try {
    ...
    if (fee_id) {
        ...
        if (feeCheck.rows.length > 0) {
            ...
            await client.query(
                `UPDATE fees SET amount_paid = $1, status = $2 WHERE id = $3`, 
