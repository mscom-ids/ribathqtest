...
const chatStorage = multer.diskStorage({
    destination: (req, file, cb) => {
        const dir = path.join(__dirname, '../../public/chat-images');
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        cb(null, dir);
    },
    filename: (req, file, cb) => {
        const suffix = Date.now() + '-' + Math.round(Math.random() * 1e9);
        cb(null, 'chat-' + suffix + path.extname(file.originalname));
    }
});
...
async function isParticipant(conversationId: string, staffId: string): Promise<boolean> {
...
async function getStaffId(user: any): Promise<string | null> {
    return cachedResult(
        makeCacheKey('chat:staff-id', { userId: user.id, email: user.email || '' }),
        60_000,
        async () => {
            const r = await db.query(
                'SELECT id FROM staff WHERE profile_id = $1 OR email = $2 LIMIT 1',
                [user.id, user.email]
            );
...
export const getConversations = async (req: Request, res: Response) => {
    try {
        ...
        if (!staffId) return res.status(404).json({ success: false, error: 'Staff not found' });

        const conversations = await cachedResult(
            makeCacheKey('chat:conversations', { staffId }),
            10_000,
            async () => {
...
export const startPrivateChat = async (req: Request, res: Response) => {
    try {
        ...
        if (!otherStaffId) return res.status(400).json({ success: false, error: 'otherStaffId required' });
        if (otherStaffId === staffId) return res.status(400).json({ success: false, error: 'Cannot chat with yourself' });
        ...
        try {
            ...
            const convRes = await client.query(
                `INSERT INTO chat_conversations (type) VALUES ('private') RETURNING id`,
            );
            const convId = convRes.rows[0].id;
            await client.query(
                'INSERT INTO chat_participants (conversation_id, staff_id) VALUES ($1, $2), ($1, $3)',
                [convId, staffId, otherStaffId]
            );
...
export const updateGroupMembers = async (req: Request, res: Response) => {
    try {
        ...
        // Verify it's a group
        const conv = await db.query('SELECT type, created_by FROM chat_conversations WHERE id = $1', [id]);
        if (conv.rows.length === 0) return res.status(404).json({ success: false, error: 'Not found' });
        if (conv.rows[0].type !== 'group') return res.status(400).json({ success: false, error: 'Not a group' });
        ...
        try {
            ...
            if (removeIds && Array.isArray(removeIds)) {
                ...
                const safeRemove = removeIds.filter((rid: string) => rid !== creator);
                if (safeRemove.length > 0) {
                    await client.query(
                        'DELETE FROM chat_participants WHERE conversation_id = $1 AND staff_id = ANY($2)',
                        [id, safeRemove]
                    );
                }
            }
...
export const getMessages = async (req: Request, res: Response) => {
    try {
        ...
        const before = req.query.before as string | undefined; // cursor: load messages before this timestamp
...
export const sendMessage = async (req: Request, res: Response) => {
    try {
        ...
        const result = await db.query(
            ...
            [id, staffId, content.trim()]
        );
        invalidateCacheByPrefix('chat:conversations');

        // Update sender's last_read_at
        await db.query(
            'UPDATE chat_participants SET last_read_at = NOW() WHERE conversation_id = $1 AND staff_id = $2',
            [id, staffId]
        );
...
export const deleteMessage = async (req: Request, res: Response) => {
    try {
        ...
        await db.query(
            'UPDATE chat_messages SET is_deleted = true, deleted_by = $1, content = NULL, image_url = NULL WHERE id = $2',
            [operator.id, messageId]
        );
...
export const getStaffList = async (req: Request, res: Response) => {
    try {
        ...
        const staff = await cachedResult(
            makeCacheKey('chat:staff-list', { staffId: staffId || '' }),
            60_000,
            async () => {
                const result = await db.query(
...
// ═══════════════════════════════════════════════════════════════════════════════
export const pollMessages = async (req: Request, res: Response) => {
    try {
        const user = (req as any).user;
        const staffId = await getStaffId(user);
        if (!staffId) return res.status(404).json({ success: false, error: 'Staff not found' });
