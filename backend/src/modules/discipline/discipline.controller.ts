...
const clearDisciplineCache = () => invalidateCacheByPrefix('discipline:');
...
export async function listIncidents(req: Request, res: Response) {
    try {
        ...
        const add = (fragment: string, value: any) => { params.push(value); where.push(fragment.replace('?', `$${params.length}`)); };
        ...
        if (req.query.student_id) add('i.student_id = ?', req.query.student_id);
        if (req.query.from) add('i.reported_at >= ?::date', req.query.from);
        if (req.query.to) add('i.reported_at < (?::date + 1)', req.query.to);
        if (req.query.search) {
        ...
        const scope = reporterScopeSql(actor.role, actor.staffId, params).replace(/^ AND /, '');
    ...
    } catch (error) { return respondError(res, error, 'Failed to load incidents'); }
}

export async function createIncident(req: Request, res: Response) {
    const client = await db.getClient();
    try {
        const input = parseIncidentCreate(req.body);
        const actor = await resolveDisciplineActor(req);
        ...
        const context = await client.query(
            `SELECT s.adm_no,ay.id academic_year_id,p.standard,p.division,o.category_id offence_category_id,
        ...
        if (!row) throw new Error('Student or offence type not found');
        if (row.offence_category_id !== input.category_id) throw new Error('Invalid problem category');
        const repeated = await client.query(
            `SELECT EXISTS(SELECT 1 FROM discipline_incidents WHERE student_id=$1 AND offence_type_id=$2
              AND deleted_at IS NULL AND status<>'cancelled' AND reported_at >= $3::timestamptz - interval '90 days') repeated`,
            [input.student_id,input.offence_type_id,input.reported_at]);
        ...
        for (const witness of input.witnesses || []) await client.query('INSERT INTO discipline_witnesses (incident_id,name,details) VALUES ($1,$2,$3)', [incident.id,witness.name,witness.details]);
        for (const evidence of input.evidence || []) await client.query(
            `INSERT INTO discipline_evidence (incident_id,uploaded_by,file_name,file_url,mime_type,file_size,visibility)
        ...
        await audit(client,{ incidentId:incident.id,studentId:incident.student_id,actorId:actor.staffId,action:'incident_created',newValue:incident,ipAddress:actor.ipAddress });
        await client.query('COMMIT'); clearDisciplineCache();
        return res.status(201).json({ success:true,incident });
    } catch (error:any) {
        await client.query('ROLLBACK');
        if (error?.code === '23505') return res.status(409).json({ success:false,error:'This incident was already submitted' });
...
export async function getIncident(req: Request, res: Response) {
    try {
        ...
        const found=await db.query(
            `SELECT i.*,s.name student_name,s.photo_url,s.father_name,s.phone,c.name category_name,o.name offence_name,
        ...
        const [witnesses,evidence,responses,actions,communications,history,audits]=await Promise.all([
            db.query('SELECT * FROM discipline_witnesses WHERE incident_id=$1 ORDER BY created_at',[incident.id]),
            db.query(`SELECT * FROM discipline_evidence WHERE incident_id=$1 ${actor.canReview ? '' : "AND visibility='student_parent'"} ORDER BY created_at`,[incident.id]),
            db.query('SELECT r.*,s.name recorded_by_name FROM discipline_student_responses r LEFT JOIN staff s ON s.id=r.recorded_by WHERE incident_id=$1 ORDER BY r.created_at DESC',[incident.id]),
            db.query(`SELECT a.*,s.name supervisor_name,COALESCE(json_agg(u ORDER BY u.created_at DESC) FILTER (WHERE u.id IS NOT NULL),'[]') updates
            ...
            db.query('SELECT c.*,s.name contacted_by_name FROM discipline_parent_communications c LEFT JOIN staff s ON s.id=c.contacted_by WHERE incident_id=$1 ORDER BY c.created_at DESC',[incident.id]),
            db.query('SELECT h.*,s.name changed_by_name FROM discipline_status_history h LEFT JOIN staff s ON s.id=h.changed_by WHERE incident_id=$1 ORDER BY h.created_at DESC',[incident.id]),
            actor.canReview ? db.query('SELECT a.*,s.name actor_name FROM discipline_audit_logs a LEFT JOIN staff s ON s.id=a.actor_id WHERE incident_id=$1 ORDER BY a.created_at DESC',[incident.id]) : Promise.resolve({rows:[]}),
        ]);
        if(!actor.canReview) delete incident.private_staff_notes;
    ...
    } catch(error){ return respondError(res,error,'Failed to load incident'); }
}
...
export async function addStudentResponse(req:Request,res:Response){
    const client=await db.getClient();
    try{const actor=await resolveDisciplineActor(req);const type=String(req.body?.response_type||'').toLowerCase();if(!['accepted','denied'].includes(type))throw new Error('Invalid student response');
        const explanation=requiredText(req.body?.explanation,'Explanation',4000);await client.query('BEGIN');
        const incident=await client.query('SELECT student_id FROM discipline_incidents WHERE id=$1 AND deleted_at IS NULL',[String(req.params.id)]);if(!incident.rows[0])throw new Error('Incident not found');
        await client.query(`INSERT INTO discipline_student_responses (incident_id,response_type,explanation,witness_details,attachment_url,recorded_by) VALUES ($1,$2,$3,$4,$5,$6)`,[String(req.params.id),type,explanation,cleanText(req.body?.witness_details,2000),cleanText(req.body?.attachment_url,1000),actor.staffId]);
        await client.query('UPDATE discipline_incidents SET student_position=$2,updated_at=now() WHERE id=$1',[String(req.params.id),type]);await changeStatus(client,String(req.params.id),'under_review',actor.staffId,'Student explanation recorded');
        await audit(client,{incidentId:String(req.params.id),studentId:incident.rows[0].student_id,actorId:actor.staffId,action:'student_response_recorded',newValue:{type,explanation},ipAddress:actor.ipAddress});
        await client.query('COMMIT');clearDisciplineCache();return res.status(201).json({success:true});
    }catch(error){await client.query('ROLLBACK');return respondError(res,error,'Failed to record response');}finally{client.release();}
}

export async function addCorrectiveAction(req:Request,res:Response){
    const client=await db.getClient();
    try{const actor=await resolveDisciplineActor(req);const actionType=requiredText(req.body?.action_type,'Action type',160);await client.query('BEGIN');
        const incident=await client.query('SELECT student_id FROM discipline_incidents WHERE id=$1 AND deleted_at IS NULL',[String(req.params.id)]);if(!incident.rows[0])throw new Error('Incident not found');
        const action=await client.query(`INSERT INTO discipline_actions (incident_id,action_type,instructions,assigned_by,supervisor_id,due_date) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,[String(req.params.id),actionType,cleanText(req.body?.instructions,2000),actor.staffId,cleanText(req.body?.supervisor_id,60),cleanText(req.body?.due_date,20)]);
        await changeStatus(client,String(req.params.id),'action_assigned',actor.staffId,`${actionType} assigned`);await audit(client,{incidentId:String(req.params.id),studentId:incident.rows[0].student_id,actorId:actor.staffId,action:'corrective_action_assigned',newValue:action.rows[0],ipAddress:actor.ipAddress});
        await client.query('COMMIT');clearDisciplineCache();return res.status(201).json({success:true,action:action.rows[0]});
    }catch(error){await client.query('ROLLBACK');return respondError(res,error,'Failed to assign action');}finally{client.release();}
}

export async function updateCorrectiveAction(req:Request,res:Response){
    const client=await db.getClient();
    try{const actor=await resolveDisciplineActor(req);const status=parseActionStatus(req.body?.status);await client.query('BEGIN');const current=await client.query('SELECT * FROM discipline_actions WHERE id=$1 FOR UPDATE',[String(req.params.actionId)]);if(!current.rows[0])throw new Error('Corrective action not found');
        const updated=await client.query(`UPDATE discipline_actions SET status=$2,completion_note=$3,completion_evidence_url=$4,completed_at=CASE WHEN $2='completed' THEN now() ELSE completed_at END,updated_at=now() WHERE id=$1 RETURNING *`,[String(req.params.actionId),status,cleanText(req.body?.note,2000),cleanText(req.body?.evidence_url,1000)]);
        await client.query('INSERT INTO discipline_action_updates (action_id,status,note,evidence_url,updated_by) VALUES ($1,$2,$3,$4,$5)',[String(req.params.actionId),status,cleanText(req.body?.note,2000),cleanText(req.body?.evidence_url,1000),actor.staffId]);
        await audit(client,{incidentId:current.rows[0].incident_id,actorId:actor.staffId,action:'corrective_action_updated',oldValue:current.rows[0],newValue:updated.rows[0],ipAddress:actor.ipAddress});await client.query('COMMIT');clearDisciplineCache();return res.json({success:true,action:updated.rows[0]});
    }catch(error){await client.query('ROLLBACK');return respondError(res,error,'Failed to update action');}finally{client.release();}
}

export async function closeIncident(req:Request,res:Response){
    const client=await db.getClient();
    try{const actor=await resolveDisciplineActor(req);await client.query('BEGIN');const incident=await client.query('SELECT * FROM discipline_incidents WHERE id=$1 AND deleted_at IS NULL FOR UPDATE',[String(req.params.id)]);if(!incident.rows[0])throw new Error('Incident not found');
        const pending=await client.query("SELECT count(*)::int count FROM discipline_actions WHERE incident_id=$1 AND status NOT IN ('completed','cancelled')",[String(req.params.id)]);if(pending.rows[0].count>0&&!req.body?.force)throw new Error('Complete or cancel pending corrective actions before closing');
        await changeStatus(client,String(req.params.id),'completed',actor.staffId,cleanText(req.body?.note,2000));await audit(client,{incidentId:String(req.params.id),studentId:incident.rows[0].student_id,actorId:actor.staffId,action:'incident_closed',newValue:{note:cleanText(req.body?.note,2000)},ipAddress:actor.ipAddress});await client.query('COMMIT');clearDisciplineCache();return res.json({success:true});
