...
exports.resolveHifzEntryEligibility = resolveHifzEntryEligibility;
exports.getHifzStudentMonthRegister = getHifzStudentMonthRegister;
const quran_data_1 = require("../utils/quran-data");
const quran_juz_1 = require("../utils/quran-juz");
const INDIA_TIMEZONE = 'Asia/Kolkata';
const HIFZ_MODES_BY_STAGE = {
...
function normalizeStandard(value) {
    ...
    if (standard.endsWith(' Standard'))
        return standard.replace(' Standard', '');
    if (standard === 'Hifz Only')
        return 'Hifz';
    return standard;
}
...
function isCancellationForStudent(cancellation, student) {
    if (!cancellation)
        return false;
...
function portionValue(portion) {
    if (portion === 'Full')
        return 1;
    if (portion?.includes('Half'))
        return 0.5;
    if (portion?.startsWith('Q'))
        return 0.25;
    return 0;
}
...
async function loadStudent(db, studentId, academicYearId) {
    const result = await db.query(`SELECT s.adm_no,
                s.name,
         ...
         LIMIT 1`, [studentId, academicYearId]);
    if (!result.rows[0])
        return null;
    ...
    return {
        ...
        group_ids: (row.group_ids || []).map((groupId) => String(groupId)),
    };
}
async function loadSchedules(db, startDate, endDate, academicYearId) {
    const result = await db.query(`SELECT a.id, a.mentor_id, a.name, a.standards, a.day_of_week, a.start_time, a.end_time,
         ...
         GROUP BY a.id
         ORDER BY a.start_time, a.id`, [startDate, endDate, academicYearId]);
    return result.rows;
}
function selectApplicableSchedule(options) {
    ...
    if (requested)
        return candidates.find((schedule) => schedule.id === requested) || null;
    ...
    const pickPreferringPresent = (pool) => {
    ...
    if (date < indiaNow.date)
        return pickPreferringPresent(candidates);
    if (date > indiaNow.date)
        return null;
...
function applicableSchedulesForStudent(schedules, student, date) {
    return schedules
...
async function resolveHifzEntryEligibility(options) {
    ...
    const dayMarks = await options.db.query(`SELECT schedule_id, status, marked_by
         FROM student_attendance_marks
         WHERE student_id = $1 AND date = $2::date`, [options.studentId, entryDate]);
    const presentSessionIds = new Set(dayMarks.rows
    ...
    const [marks, cancellations] = await Promise.all([
        ...
        options.db.query(`SELECT cancelled_standards
             FROM attendance_cancellations
             WHERE schedule_id = $1 AND date = $2::date
             LIMIT 1`, [schedule.id, entryDate]),
    ]);
...
function entriesForDay(logs, stage) {
    const entries = {
        newHifz: [], recentRevision: [], juzRevision: [], newJuzRevision: [], oldJuzRevision: [],
    };
    for (const log of logs) {
        const key = modeKey(log.mode);
        if (key in entries && HIFZ_MODES_BY_STAGE[stage].includes(log.mode))
            entries[key].push(log);
    }
    return entries;
}
function monthSummary(logs, student, completedJuz = 0) {
    ...
    return {
        newHifzPages: (0, quran_data_1.calculateCoveredPagesFromLogs)(newHifz),
...
async function getHifzStudentMonthRegister(options) {
    ...
    const [student, logsResult, schedules, marksResult, cancellationsResult, lifetimeNewLogsResult] = await Promise.all([
        loadStudent(options.db, options.studentId, options.academicYearId || null),
        options.db.query(`SELECT hl.*, recorder.name AS recorded_by_name
             FROM hifz_logs hl
             ...
             ORDER BY hl.entry_date, hl.created_at, hl.id`, [options.studentId, start, end]),
        loadSchedules(options.db, start, end, options.academicYearId || null),
        options.db.query(`SELECT schedule_id, date, status, marked_by
             FROM student_attendance_marks
             WHERE student_id = $1 AND date BETWEEN $2::date AND $3::date`, [options.studentId, start, end]),
        options.db.query(`SELECT schedule_id, date, cancelled_standards
             FROM attendance_cancellations
             WHERE date BETWEEN $1::date AND $2::date`, [start, end]),
        options.db.query(`SELECT surah_name, start_v, end_v
             FROM hifz_logs
             WHERE student_id = $1 AND mode = 'New Verses' AND deleted_at IS NULL`, [options.studentId]),
    ]);
    if (!student) {
    ...
    const completedJuz = (0, quran_juz_1.countCompletedJuz)(lifetimeNewLogsResult.rows);
    if (completedJuz >= 30) {
    ...
    marksResult.rows.forEach((mark) => {
        ...
        if (String(mark.status).toUpperCase() === 'PRESENT'
            ...
            if (!presentByDate.has(key))
                presentByDate.set(key, new Set());
