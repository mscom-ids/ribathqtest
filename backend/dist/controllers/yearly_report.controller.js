...
exports.getAvailableClasses = exports.getClassYearlyReport = exports.getStudentYearlyReport = void 0;
...
function scheduleWindow(scheduleEffectiveFrom, scheduleEffectiveUntil, reportStart, reportEnd) {
...
function dayOfWeekMatch(dateStr, dayOfWeek) {
    return new Date(`${dateStr}T00:00:00`).getDay() === dayOfWeek;
}
// ──────────────────────────────────────────────────────────────────────────────
// Core: fetch all data for one student in one period
// ──────────────────────────────────────────────────────────────────────────────
async function fetchStudentYearlyData(studentId, reportStart, reportEnd) {
    const [schedulesRes, cancellationsRes, attendanceMarksRes, studentMarksRes, hifzLogsRes, leavesRes, examsRes, mentorHistoryRes, institutionalLeavesRes,] = await Promise.all([
        // All attendance schedules that overlap the report window
        db_1.db.query(`SELECT id, class_type, name, standards, day_of_week, start_time, end_time,
              effective_from, effective_until
       FROM attendance_schedules
       WHERE (effective_until IS NULL OR effective_until >= $1::date)
         AND effective_from <= $2::date
       ORDER BY class_type, day_of_week, start_time`, [reportStart, reportEnd]),
        // Cancellations in window
        db_1.db.query(`SELECT schedule_id, date, cancelled_standards
       FROM attendance_cancellations
       WHERE date >= $1 AND date <= $2`, [reportStart, reportEnd]),
       ...
       WHERE date >= $1 AND date <= $2`, [reportStart, reportEnd]),
        ...
        db_1.db.query(`SELECT schedule_id, date, status, marked_by
       FROM student_attendance_marks
       WHERE student_id = $1 AND date >= $2 AND date <= $3`, [studentId, reportStart, reportEnd]),
        // Hifz logs
        db_1.db.query(`SELECT id, mode, entry_date, surah_name, start_v, end_v, start_page, end_page,
       ...
       ORDER BY start_datetime ASC`, [studentId, reportStart, reportEnd]),
        // Exam results for this student (linked to academic year via exam)
        db_1.db.query(`SELECT er.id, er.marks_obtained, er.max_marks, er.grade, er.remarks,
       ...
       ORDER BY e.start_date ASC, es.subject_name ASC`, [studentId, reportStart, reportEnd]),
        // Hifz mentor history during period
        db_1.db.query(`SELECT hmh.id, hmh.assigned_from, hmh.assigned_until, hmh.notes,
       ...
       ORDER BY hmh.assigned_from ASC`, [studentId, reportStart, reportEnd]).catch(() => ({ rows: [] })), // graceful if table doesn't exist yet
       ...
       WHERE start_datetime <= $2::timestamptz
         AND end_datetime >= $1::timestamptz`, [reportStart, reportEnd]).catch(() => ({ rows: [] })),
    ]);
    return {
...
function computeAttendanceBreakdowns(student, reportStart, reportEnd, data) {
    ...
    const markedSet = new Set(); // schedule|date that were marked by mentor
    data.attendanceMarks.forEach(m => markedSet.add(`${m.schedule_id}|${toDateKey(m.date)}`));
    const studentMarkMap = new Map();
    ...
    function studentInSchedule(schedule) {
        ...
        if (type === 'hifz')
            return true; // Hifz attendance is per-session, student is included if hifz student
    ...
    function isInstLeaveCancellation(schedule, dateStr) {
        ...
        for (const leave of data.institutionalLeaves) {
            ...
            if (!(schedStart < lEnd && schedEnd > lStart))
                continue;
    ...
    for (const schedule of data.schedules) {
        if (!studentInSchedule(schedule))
            continue;
        const win = scheduleWindow(toDateKey(schedule.effective_from), schedule.effective_until ? toDateKey(schedule.effective_until) : null, reportStart, reportEnd);
        if (!win)
            continue;
...
// ──────────────────────────────────────────────────────────────────────────────
function computeHifzProgress(logs) {
    const byMode = new Map();
    for (const log of logs) {
        const mode = log.mode || 'Unknown';
        const entry = byMode.get(mode) || { count: 0, pages: 0, verses: 0, juz: new Set() };
        entry.count++;
        if (mode === 'New Verses') {
            if (log.start_page && log.end_page)
                entry.pages += Math.max(0, Number(log.end_page) - Number(log.start_page) + 1);
            if (log.start_v && log.end_v)
                entry.verses += Math.max(0, Number(log.end_v) - Number(log.start_v) + 1);
        }
        if (log.juz_number)
            entry.juz.add(Number(log.juz_number));
...
const getStudentYearlyReport = async (req, res) => {
    try {
        ...
        // Fetch student
        const studentRes = await db_1.db.query(`SELECT s.adm_no, s.name, s.batch_year, s.standard, s.status, s.photo_url,
       ...
       WHERE s.adm_no = $1`, [studentId, academicContext.academicYearId]);
        ...
        const requestedStart = String(Array.isArray(req.query.start_date) ? req.query.start_date[0] : req.query.start_date || academicBounds.start_date || '');
        const requestedEnd = String(Array.isArray(req.query.end_date) ? req.query.end_date[0] : req.query.end_date || academicBounds.end_date || '');
        ...
        if (!student.exit_date) {
            student.exit_date = (0, report_window_1.studentExitDate)(student);
        }
        const reportWindow = (0, report_window_1.resolveStudentReportWindow)(student, requestedStart, requestedEnd, academicBounds);
        ...
        const rStart = String(reportWindow.effective_start_date);
        const rEnd = String(reportWindow.effective_end_date);
        // Determine if this is a Hifz student
        const isHifzStudent = !!(student.hifz_mentor_id ||
        ...
        const mentorHistory = data.mentorHistory.map((h) => ({
...
exports.getStudentYearlyReport = getStudentYearlyReport;
// ──────────────────────────────────────────────────────────────────────────────
...
const getClassYearlyReport = async (req, res) => {
    try {
        ...
        let studentsRes;
        if (type === 'hifz') {
            studentsRes = await db_1.db.query(`SELECT s.adm_no, s.name, s.photo_url, s.admission_date, s.exit_date,
         ...
         ORDER BY st.name NULLS LAST, s.name ASC`, [academicContext.academicYearId]);
        }
        else if (type === 'school') {
            ...
            if (standard) {
                ...
                where += ` AND se.school_standard = $${params.length}`;
            }
            if (section) {
                params.push(section);
                where += ` AND se.school_section = $${params.length}`;
            }
            studentsRes = await db_1.db.query(`SELECT s.adm_no, s.name, s.photo_url, s.admission_date, s.exit_date,
         ...
         LEFT JOIN staff sm ON s.school_mentor_id = sm.id
         WHERE ${where}
         ORDER BY se.school_standard, se.school_section, s.name ASC`, params);
        }
        else {
            const params = [academicContext.academicYearId];
            ...
            if (standard) {
                params.push(standard);
                where += ` AND me.madrasa_standard = $${params.length}`;
            }
            if (section) {
                params.push(section);
                where += ` AND me.madrasa_section = $${params.length}`;
            }
            studentsRes = await db_1.db.query(`SELECT s.adm_no, s.name, s.photo_url, s.admission_date, s.exit_date,
         ...
         LEFT JOIN staff mm ON s.madrasa_mentor_id = mm.id
         WHERE ${where}
         ORDER BY me.madrasa_standard, me.madrasa_section, s.name ASC`, params);
        }
        // For each student, compute a quick attendance summary for the year
        const summaries = await Promise.all(studentsRes.rows.map(async (student) => {
            const exitDate = student.exit_date ? toDateKey(student.exit_date) : (0, report_window_1.studentExitDate)(student);
            ...
            const marksRes = await db_1.db.query(`SELECT sam.status, sch.class_type
           ...
           WHERE sam.student_id = $1 AND sam.date >= $2 AND sam.date <= $3`, [student.adm_no, effStart, effEnd]);
            ...
            const present = relevant.filter((m) => ['present', 'late'].includes(String(m.status || '').toLowerCase())).length;
...
exports.getClassYearlyReport = getClassYearlyReport;
// ──────────────────────────────────────────────────────────────────────────────
...
const getAvailableClasses = async (req, res) => {
    try {
        ...
        const [schoolRes, madrasaRes, hifzMentorsRes] = await Promise.all([
         ...
         ORDER BY school_standard, school_section`, [academicContext.academicYearId]),
            db_1.db.query(`SELECT DISTINCT madrasa_standard AS standard, madrasa_section AS section
         FROM student_madrasa_enrollments
         WHERE academic_year_id = $1 AND status = 'active'
           AND madrasa_standard IS NOT NULL
         ORDER BY madrasa_standard, madrasa_section`, [academicContext.academicYearId]),
            db_1.db.query(`SELECT DISTINCT st.id AS mentor_id, st.name AS mentor_name,
