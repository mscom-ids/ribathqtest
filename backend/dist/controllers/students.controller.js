...
exports.downloadStudentsExcel = exports.exportStudents = exports.updateStudent = exports.createStudent = exports.getStudentById = exports.getStudentInsideOutsideSummary = exports.getStudentCounts = exports.getAllStudents = void 0;
...
async function hasStudentCurrentPresenceTable(client) {
...
async function getActiveOperationalRecords(client, studentId) {
    const [leaveRes, hasPresenceTable] = await Promise.all([
        client.query(`SELECT COUNT(*)::integer AS count
       FROM student_leaves
       WHERE student_id = $1 AND status = ANY($2::text[])`, [studentId, ACTIVE_OPERATIONAL_LEAVE_STATUSES]),
    ...
    const presenceRes = hasPresenceTable
       ...
       WHERE student_id = $1 AND status IN ('outside', 'on-campus')`, [studentId])
        : { rows: [{ count: 0 }] };
    return {
...
function hasActiveOperationalRecords(records) {
    return records.active_leaves > 0 || records.active_presence > 0 || records.active_hostel > 0;
}
async function closeActiveOperationalRecords(client, studentId, userId) {
    const cancelledLeavesRes = await client.query(`UPDATE student_leaves
     SET status = 'cancelled',
         actual_return_datetime = NULL,
         return_status = NULL,
         updated_at = NOW()
     WHERE student_id = $1 AND status = ANY($2::text[])
     RETURNING id, status`, [studentId, ACTIVE_OPERATIONAL_LEAVE_STATUSES]);
    if (await hasStudentCurrentPresenceTable(client)) {
        await client.query(`DELETE FROM student_current_presence
       WHERE student_id = $1 AND status IN ('outside', 'on-campus')`, [studentId]);
    }
...
const getAllStudents = async (req, res) => {
    try {
        ...
        if (shouldPaginate) {
            ...
            const cacheKey = (0, server_cache_1.makeCacheKey)('students:list', {
                role: user?.role || '',
            ...
            const payload = await (cacheable
                ? (0, server_cache_1.cachedResult)(cacheKey, 60000, async () => {
                    const [result, countRes] = await Promise.all([
                        db_1.db.query(query, pagedParams),
                        includeCount ? db_1.db.query(countQuery, params) : Promise.resolve({ rows: [{ total: null }] }),
                    ]);
            ...
            return res.json({
                success: true,
                students: payload.rows.map((student) => (0, academic_year_1.applyAcademicSnapshot)(student, snapshotMap.get(student.adm_no))),
        ...
        res.json({
            success: true,
            students: rows.map((student) => (0, academic_year_1.applyAcademicSnapshot)(student, snapshotMap.get(student.adm_no))),
            academic_year_mode: academicContext.mode,
        });
    }
    catch (err) {
...
const getStudentCounts = async (_req, _res) => {
    try {
        const counts = await (0, server_cache_1.cachedResult)('students:counts', 5 * 60000, async () => {
            const presencePromise = hasStudentCurrentPresenceTable(db_1.db).then((hasPresenceTable) => {
                if (hasPresenceTable) {
...
const getStudentInsideOutsideSummary = async (req, res) => {
    try {
        const academicContext = await (0, academic_year_1.getAcademicYearContext)(db_1.db, req.query.academic_year_id);
        const academicYearId = academicContext.academicYearId;
        const buildQuery = (outsideSource) => `
    ...
    `;
        let result;
        try {
            result = await db_1.db.query(buildQuery(presenceOutsideSource), [academicYearId]);
        }
        catch (presenceErr) {
            if (presenceErr?.code !== '42P01')
                throw presenceErr;
            result = await db_1.db.query(buildQuery(leaveOutsideSource), [academicYearId]);
        }
        ...
        const classSortValue = (standard) => {
            const lower = standard.toLowerCase();
            if (lower.includes('plus one'))
                return 11;
            if (lower.includes('plus two'))
                return 12;
...
exports.getStudentInsideOutsideSummary = getStudentInsideOutsideSummary;
...
const updateStudent = async (req, res) => {
    try {
        ...
        try {
            ...
            const currentRes = await client.query(`SELECT adm_no, status, hifz_mentor_id FROM students WHERE adm_no = $1 FOR UPDATE`, [studentId]);
      ...
      `;
            ...
            if (newHifzMentorId !== undefined && String(newHifzMentorId || '') !== String(oldHifzMentorId || '')) {
             ...
             WHERE student_id = $1 AND assigned_until IS NULL`, [studentId]);
               ...
               ON CONFLICT DO NOTHING`, [studentId, newHifzMentorId]);
                    }
             ...
             WHERE student_id = $1`, [studentId, newHifzMentorId]);
                }
                catch (hifzErr) {
