...
exports.correctStudentLeave = void 0;
 ...
 */
const correctStudentLeave = async (req, res) => {
    const user = req.user;
    ...
    const parseText = (value, label, maxLength, required = false) => {
    ...
    try {
        ...
        if (MENTOR_ROLES.includes(user.role)) {
            ...
            const assignedRes = await client.query(`SELECT 1
                 FROM students
                 WHERE adm_no = $1
        ...
        if (has('reason'))
            addUpdate('reason', correctedReason);
        ...
        if (has('remarks'))
            addUpdate('remarks', correctedRemarks);
        if (has('companion_name'))
            addUpdate('companion_name', correctedCompanionName);
        if (has('companion_relationship'))
            addUpdate('companion_relationship', correctedCompanionRelationship);
        let correctedReturnStatus = leave.return_status;
        ...
        if (returnChangeRequested && correctedReturn) {
            if (leave.return_movement_id) {
                await client.query(`UPDATE student_movements
                     SET timestamp = $2, is_late = $3
                     WHERE id = $1`, [leave.return_movement_id, correctedReturn.toISOString(), correctedReturnStatus === 'late']);
            }
            else {
                await client.query(`
...
exports.correctStudentLeave = correctStudentLeave;
