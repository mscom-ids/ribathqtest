...
exports.getAllLeaves = exports.getOutsideStudents = exports.getActiveLeaves = exports.getMovementHistory = exports.recordReturn = exports.getLeavesFilter = exports.createGroupLeave = exports.createPersonalLeave = exports.bulkRecordGroupReturn = exports.getGroupLeaveStudents = exports.bulkRecordReturn = exports.deleteInstitutionalLeave = exports.getInstitutionalLeaveStudents = exports.getInstitutionalLeaves = exports.markInstitutionalExit = exports.getInstitutionalEligibleStudents = exports.createInstitutionalLeave = exports.getEligibleStudents = void 0;
...
function institutionalCancellationReason(id) {
...
async function applyInstitutionalAttendanceCancellations(client, leave) {
    ...
    if (!leave.is_entire_institution && targetClasses.length === 0)
        return 0;
    const schedulesRes = await client.query(`SELECT id, standards, day_of_week, start_time, end_time, effective_from, effective_until
         FROM attendance_schedules
         WHERE (is_deleted = false OR is_deleted IS NULL)
           AND (effective_from IS NULL OR effective_from <= $2::date)
           AND (effective_until IS NULL OR effective_until >= $1::date)`, [dateKeys[0], dateKeys[dateKeys.length - 1]]);
    ...
    for (const dateKey of dateKeys) {
        ...
        for (const schedule of schedulesRes.rows) {
            ...
            if (!doRangesOverlap(scheduleStart, scheduleEnd, leaveStart, leaveEnd))
                continue;
    ...
    if (rows.length === 0)
        return 0;
    await client.query(`INSERT INTO attendance_cancellations (schedule_id, date, reason, cancelled_by, cancelled_standards)
...
const getEligibleStudents = async (req, res) => {
    try {
        ...
        if (MENTOR_ROLES.includes(user.role)) {
            ...
            const parentRes = await db_1.db.query('SELECT parent_id FROM profiles WHERE id = $1', [user.id]);
 ...
 *  INSTITUTIONAL LEAVE
 *  ============================= */
const createInstitutionalLeave = async (req, res) => {
    try {
        ...
        try {
            ...
            if (exceptions && exceptions.length > 0) {
                await client.query(`INSERT INTO leave_exceptions (institutional_leave_id, student_id)
                     SELECT $1, sid FROM unnest($2::text[]) AS t(sid)`, [inst_id, exceptions]);
            }
...
exports.createInstitutionalLeave = createInstitutionalLeave;
const getInstitutionalEligibleStudents = async (req, res) => {
    try {
        const { id } = req.params;
        const user = req.user;
        const leaveRes = await db_1.db.query(`SELECT * FROM institutional_leaves WHERE id = $1`, [id]);
...
exports.getInstitutionalEligibleStudents = getInstitutionalEligibleStudents;
const markInstitutionalExit = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        try {
            ...
            const allowedIds = studentsRes.rows.map((r) => r.adm_no);
            ...
            const openCheck = await client.query(`SELECT student_id FROM student_leaves WHERE student_id = ANY($1::text[]) AND status = 'outside'`, [allowedIds]);
            if (openCheck.rows.length > 0) {
                throw new Error('One or more selected students are already outside campus');
            }
            const existingCheck = await client.query(`SELECT student_id FROM student_leaves WHERE student_id = ANY($1::text[]) AND institutional_leave_id = $2`, [allowedIds, id]);
            if (existingCheck.rows.length > 0) {
                throw new Error('Exit has already been recorded for one or more selected students');
            }
            const slBulk = await client.query(`INSERT INTO student_leaves
                 ...
                 RETURNING id, student_id`, [allowedIds, id, exit_datetime, inst.end_datetime, inst.name, companion_name.trim(), companion_relationship.trim(), user.id]);
            const movementStudentIds = slBulk.rows.map((r) => r.student_id);
            const movementLeaveIds = slBulk.rows.map((r) => r.id);
            await client.query(`INSERT INTO student_movements (student_id, leave_id, direction, timestamp, recorded_by)
                 SELECT sid, lid, 'out', $1, $2
                 FROM unnest($3::text[], $4::uuid[]) AS t(sid, lid)`, [exit_datetime, user.id, movementStudentIds, movementLeaveIds]);
            await client.query('COMMIT');
            invalidateLeaveCaches();
...
exports.markInstitutionalExit = markInstitutionalExit;
const getInstitutionalLeaves = async (req, res) => {
    try {
        const leaves = await (0, server_cache_1.cachedResult)('leaves:institutional', 10000, async () => {
            const query = `
...
exports.getInstitutionalLeaves = getInstitutionalLeaves;
const getInstitutionalLeaveStudents = async (req, res) => {
    try {
        const { id } = req.params;
        const query = `
...
exports.getInstitutionalLeaveStudents = getInstitutionalLeaveStudents;
const deleteInstitutionalLeave = async (req, res) => {
    try {
...
exports.deleteInstitutionalLeave = deleteInstitutionalLeave;
const bulkRecordReturn = async (req, res) => {
    try {
        const { id } = req.params;
        const { return_datetime, standard, leave_ids } = req.body;
        const user = req.user;
        if (!return_datetime)
            return res.status(400).json({ success: false, error: "Return time required" });
        const client = await db_1.db.getClient();
        try {
            await client.query('BEGIN');
            ...
            const leaveIds = pending.rows.map((r) => r.id);
            const studentIds = pending.rows.map((r) => r.student_id);
            await client.query(`UPDATE student_leaves
                 SET status = 'returned',
                     ...
                     updated_at = NOW()
                 WHERE id = ANY($2::uuid[])`, [return_datetime, leaveIds]);
            await client.query(`INSERT INTO student_movements (student_id, leave_id, direction, timestamp, is_late, recorded_by)
                 SELECT sid, lid, 'in', $1, $1::timestamptz > end_dt, $2
                 FROM unnest($3::text[], $4::uuid[], $5::timestamptz[]) AS t(sid, lid, end_dt)`, [return_datetime, user.id, studentIds, leaveIds, pending.rows.map((r) => r.end_datetime)]);
            await client.query('COMMIT');
            invalidateLeaveCaches();
...
exports.bulkRecordReturn = bulkRecordReturn;
const getGroupLeaveStudents = async (req, res) => {
    try {
        const { id } = req.params;
        const user = req.user;
        const params = [id];
...
exports.getGroupLeaveStudents = getGroupLeaveStudents;
const bulkRecordGroupReturn = async (req, res) => {
    try {
        const { id } = req.params;
        const { return_datetime, standard, leave_ids } = req.body;
        const user = req.user;
...
exports.bulkRecordGroupReturn = bulkRecordGroupReturn;
/** =============================
 *  OUT/ON CAMPUS LEAVES
 *  ============================= */
const createPersonalLeave = async (req, res) => {
    try {
        ...
        if (MENTOR_ROLES.includes(user.role)) {
            ...
            const assignedRes = await db_1.db.query(`SELECT 1 FROM students
                 WHERE adm_no = $1
                   AND (hifz_mentor_id = $2 OR school_mentor_id = $2 OR madrasa_mentor_id = $2)
                 LIMIT 1`, [student_id, staffId]);
...
exports.createPersonalLeave = createPersonalLeave;
const createGroupLeave = async (req, res) => {
    try {
        ...
        try {
            ...
            const group_id = crypto_1.default.randomUUID();
            ...
            const slBulk = await client.query(`INSERT INTO student_leaves
                 ...
                 FROM unnest($1::text[]) AS t(sid)
                 RETURNING id, student_id`, [targetStudents, leave_type, start_datetime, end_datetime, reason_category, remarks,
                companion_name || null, companion_relationship || null, group_id, group_type, String(group_value), user.id]);
            const movStudentIds = slBulk.rows.map((r) => r.student_id);
            const movLeaveIds = slBulk.rows.map((r) => r.id);
            await client.query(`INSERT INTO student_movements (student_id, leave_id, direction, timestamp, recorded_by)
                 SELECT sid, lid, 'out', $1, $2
                 FROM unnest($3::text[], $4::uuid[]) AS t(sid, lid)`, [start_datetime, user.id, movStudentIds, movLeaveIds]);
            await client.query('COMMIT');
            invalidateLeaveCaches();
...
exports.createGroupLeave = createGroupLeave;
const getLeavesFilter = async (req, res) => {
    try {
        ...
        let staffId = null;
        if (MENTOR_ROLES.includes(user.role)) {
            staffId = await (0, staff_utils_1.getStaffId)(req);
            if (!staffId) {
                return res.status(403).json({ success: false, error: 'Staff profile not found' });
            }
        }
        const cacheKey = (0, server_cache_1.makeCacheKey)('leaves:filter', {
            type: String(type || ''),
            role: user.role,
            staffId: staffId || 'all',
        ...
        const groupedLeaves = await (0, server_cache_1.cachedResult)(cacheKey, 10000, async () => {
...
exports.getLeavesFilter = getLeavesFilter;
/** =============================
 *  MOVEMENT & RETURN
 *  ============================= */
    ...
    try {
        ...
        try {
            ...
            if (MENTOR_ROLES.includes(user.role)) {
                ...
                const assignedRes = await client.query(`SELECT 1 FROM students
                     WHERE adm_no = $1
                       AND (hifz_mentor_id = $2 OR school_mentor_id = $2 OR madrasa_mentor_id = $2)
                     LIMIT 1`, [leave.student_id, staffId]);
                if (assignedRes.rows.length === 0) {
                    await client.query('ROLLBACK');
                    return res.status(403).json({ success: false, error: 'Not authorized for this student' });
                }
            }
            if (leave.status === 'returned' || leave.status === 'completed')
                throw new Error('Leave has already been closed');
...
const getMovementHistory = async (req, res) => {
    try {
        ...
        const [result, countRes] = await Promise.all([
            db_1.db.query(query, [limit, offset]),
            db_1.db.query(`SELECT COUNT(*)::integer as total FROM student_movements`),
        ]);
        res.json({
            success: true,
...
const getActiveLeaves = async (req, res) => {
    try {
        ...
        let result;
        try {
            result = await db_1.db.query(query, params);
        }
        catch (presenceErr) {
            ...
            result = await db_1.db.query(fallbackQuery, fallbackParams);
        }
...
const getOutsideStudents = async (req, res) => {
    try {
        ...
        const cacheKey = (0, server_cache_1.makeCacheKey)('leaves:outside-students', {
            role: user.role,
...
exports.getOutsideStudents = getOutsideStudents;
/**
 * GET /leaves
...
const getAllLeaves = async (req, res) => {
    try {
        ...
        const cacheKey = (0, server_cache_1.makeCacheKey)('leaves:all', { limit, offset, search });
        const payload = await (0, server_cache_1.cachedResult)(cacheKey, 15000, async () => {
            const params = [];
            let whereClause = '';
            if (search) {
                params.push(`%${search}%`);
