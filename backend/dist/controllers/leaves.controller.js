    ...
    try {
        ...
        try {
            ...
            if (!is_entire_institution && target_classes && target_classes.length > 0) {
                const placeholders = target_classes.map((_, i) => `$${i + 1}`).join(',');
            ...
            if (targetStudents.length > 0) {
                const slBulk = await client.query(`INSERT INTO student_leaves
                     ...
                     RETURNING id, student_id`, [targetStudents, inst_id, start_datetime, end_datetime]);
                ...
                await client.query(`INSERT INTO student_movements (student_id, leave_id, direction, timestamp, recorded_by)
                     ...
                     FROM unnest($3::text[], $4::uuid[]) AS t(sid, lid)`, [start_datetime, user.id, movementStudentIds, movementLeaveIds]);
            }
    ...
    try {
        ...
        try {
            ...
            await client.query(`INSERT INTO student_movements (student_id, leave_id, direction, timestamp, recorded_by)
                 ...
                 FROM unnest($3::text[], $4::uuid[]) AS t(sid, lid)`, [return_datetime, user.id, studentIds, leaveIds]);
            await client.query('COMMIT');
...
const getLeavesFilter = async (req, res) => {
    try {
        ...
        `;
        const result = await db_1.db.query(query, [type]);
