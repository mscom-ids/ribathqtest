...
const getOutsideStudents = async (req, res) => {
    try {
        ...
        const leavesRes = await db_1.db.query(`
            SELECT 
                sl.id as leave_id,
                sl.student_id,
                sl.leave_type,
        ...
        const leaveIds = leavesRes.rows.map((r) => r.leave_id);
        const movementsRes = await db_1.db.query(`
        ...
        // Step 3: merge
        const students = leavesRes.rows.map((row) => ({
            ...row,
