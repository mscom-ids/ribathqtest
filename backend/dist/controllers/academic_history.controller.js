...
const commitYearStart = async (req, res) => {
    ...
    try {
        ...
        const targetYearRes = await client.query(`SELECT start_date FROM academic_years WHERE id = $1 LIMIT 1`, [targetYearId]);
        const targetYearStart = targetYearRes.rows[0]?.start_date
                ...
                if (mentorChanging) {
                    try {
                        await client.query(`UPDATE hifz_mentor_history
                             ...
                             WHERE student_id = $1 AND assigned_until IS NULL`, [row.student_id, targetYearStart]);
                        if (row.to_mentor_id) {
                            await client.query(`INSERT INTO hifz_mentor_history (student_id, mentor_id, assigned_from, notes)
                                 VALUES ($1, $2, $3::date, 'Year start promotion')
                                 ON CONFLICT DO NOTHING`, [row.student_id, row.to_mentor_id, targetYearStart]);
                        }
                    }
                    catch (hErr) {
