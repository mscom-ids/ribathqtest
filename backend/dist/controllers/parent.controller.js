...
exports.submitLeaveRequest = exports.getMyChildren = exports.getParentDashboard = exports.parentLogin = void 0;
...
function formatMonthEndForDashboard(month) {
...
function getParentStudentId(req) {
    const user = req.user;
...
function groupAttendanceSessions(sessions = []) {
...
function withGroupedAttendanceSessions(summary) {
    if (!summary)
        return null;
...
function getLogPages(log) {
    const surahId = (0, quran_data_1.getSurahId)(log.surah_name || '');
    if (surahId && log.start_v && log.end_v) {
        return (0, quran_data_1.calculatePages)(surahId, Number(log.start_v), surahId, Number(log.end_v));
    }
...
function getJuzPortionValue(portion) {
    if (portion === 'Full')
        return 1;
...
function buildWeeklyHifzReport(logs, startDate, endDate) {
        ...
        if (day === 4 || index === days.length - 1) {
            const dayRows = weekDays.map((dayKey) => {
                ...
                return {
                    ...
                    recorded_by: [...new Set(dayLogs.map((l) => l.usthad_name).filter(Boolean))],
                };
            });
...
function summarizeHifzLogs(logs) {
    ...
    logs.forEach((log) => {
        ...
        if (log.mode === 'Juz Revision')
            juzRevision += getJuzPortionValue(log.juz_portion || log.juz_part);
        if (log.mode === 'Juz Revision (New)')
            newRevision += getJuzPortionValue(log.juz_portion || log.juz_part);
        if (log.mode === 'Juz Revision (Old)')
            oldRevision += getJuzPortionValue(log.juz_portion || log.juz_part);
    });
    return {
...
const parentLogin = async (req, res) => {
    try {
        ...
        if (parsedDob.getFullYear() !== Number(yearText) ||
        ...
        const result = await db_1.db.query(`SELECT adm_no, name, photo_url, batch_year, standard, dob, status
             FROM students
             WHERE LOWER(TRIM(adm_no)) = LOWER(TRIM($1))
               AND dob::date = $2::date
             LIMIT 1`, [admissionNo, dob]);
    ...
    try {
        ...
        const [studentResult, logsResult, monthLogsResult, lifetimeNewLogsResult, reportsResult, examRowsResult] = await Promise.all([
            db_1.db.query(`SELECT
                 ...
                 LIMIT 1`, [studentId]),
            db_1.db.query(`SELECT hl.id, hl.student_id, hl.mode, hl.entry_date,
            ...
            db_1.db.query(`SELECT hl.id, hl.student_id, hl.mode, hl.entry_date,
                 ...
                 ORDER BY hl.entry_date ASC, hl.created_at ASC`, [studentId, fullMonth.startDate, fullMonth.endDate]),
            db_1.db.query(`SELECT surah_name, start_v, end_v, start_page, end_page
                 FROM hifz_logs
                 WHERE student_id = $1 AND mode = 'New Verses'
                 ORDER BY entry_date ASC, created_at ASC`, [studentId]),
            db_1.db.query(`SELECT report_month, hifz_pages, recent_pages, juz_revision,
            ...
            db_1.db.query(`SELECT
                 ...
                 ORDER BY e.start_date DESC NULLS LAST, e.created_at DESC, es.order_index NULLS LAST, es.created_at`, [studentId]),
        ]);
        ...
        const [attendanceSummaries, hifzAttendanceSummaries] = await Promise.all([
            (0, attendance_report_1.getStudentAttendanceSummaries)(db_1.db, [{
        ...
        const hifzPoints = (0, hifz_calculator_1.calculateHifzReportPoints)(monthLogs, [], {
        ...
        const monthHafizModes = monthLogs.some((log) => log.mode === 'Juz Revision (New)' || log.mode === 'Juz Revision (Old)');
        const monthRegularModes = monthLogs.some((log) => log.mode === 'New Verses' || log.mode === 'Recent Revision' || log.mode === 'Juz Revision');
        const recentHafizModes = logsResult.rows.some((log) => log.mode === 'Juz Revision (New)' || log.mode === 'Juz Revision (Old)');
...
const getMyChildren = async (req, res) => {
    try {
        ...
        const result = await db_1.db.query(`SELECT adm_no, name, photo_url, batch_year, standard, dob
             FROM students
