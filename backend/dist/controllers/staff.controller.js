...
const getMyStudentsWithStats = async (req, res) => {
    try {
        ...
        const activeLeaveResult = await db_1.db.query(`SELECT student_id, leave_type, reason_category, remarks, end_datetime
             FROM student_leaves 
        ...
        const logsResult = await db_1.db.query(`SELECT student_id, mode, start_page, end_page, juz_portion, entry_date
             FROM hifz_logs WHERE student_id = ANY($1) AND entry_date = $2`, [studentIds, todayDate]);
        const logs = logsResult.rows;
        // Fetch today's attendance
        const attResult = await db_1.db.query(`SELECT student_id, status FROM attendance WHERE student_id = ANY($1) AND date = $2`, [studentIds, todayDate]);
        const attendance = attResult.rows;
        // Fetch ANY active outgoing delegations from this mentor (could be specific students or ALL)
        const outgoingDelegationsRes = await db_1.db.query(`
