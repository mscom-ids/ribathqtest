...
exports.issueDelegationToken = exports.revokeDelegation = exports.updateDelegationStatus = exports.getAdminAllRequests = exports.getAssignedToMe = exports.getOutgoingRequests = exports.createDelegationRequest = exports.getAssignableMentors = void 0;
...
const getAssignableMentors = async (req, res) => {
    try {
...
exports.getAssignableMentors = getAssignableMentors;
...
const createDelegationRequest = async (req, res) => {
    try {
        ...
        const targetRes = await db_1.db.query(`
            SELECT id
            FROM staff
            WHERE id = $1
              AND COALESCE(is_active, true) = true
        ...
        if (student_id) {
            const studentRes = await db_1.db.query(`
                SELECT adm_no
                FROM students
                WHERE adm_no = $1
                  AND status = 'active'
        ...
        (0, server_cache_1.invalidateCacheByPrefix)('delegations:admin');
...
const getAdminAllRequests = async (req, res) => {
    try {
        ...
        if (req.query.count_only === 'true') {
            const pending_count = await (0, server_cache_1.cachedResult)('delegations:admin:pending-count', 15000, async () => {
        ...
        if (req.query.pending_only === 'true') {
            ...
            const requests = await (0, server_cache_1.cachedResult)((0, server_cache_1.makeCacheKey)('delegations:admin:pending-preview', { limit }), 30000, async () => {
                const result = await db_1.db.query(`
        ...
        const requests = await (0, server_cache_1.cachedResult)((0, server_cache_1.makeCacheKey)('delegations:admin:all', { user: req.user?.id || 'system' }), 15000, async () => {
            const result = await db_1.db.query(`
                    SELECT d.*,
