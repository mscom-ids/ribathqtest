...
exports.getMentorReports = exports.getStudentReports = void 0;
...
const getStudentReports = async (req, res) => {
    try {
        ...
        const studentsRes = await db_1.db.query(`SELECT s.adm_no, s.name, s.batch_year, s.standard, s.status, s.photo_url,
       ...
       ORDER BY s.name ASC`);
        ...
        const attendanceRes = await db_1.db.query(`SELECT 
         student_id,
       ...
       GROUP BY student_id`, [monthStart, monthEnd]);
        ...
        const hifzRes = await db_1.db.query(`SELECT DISTINCT ON (student_id) student_id, juz_number as juz, current_page as page
       FROM hifz_progress 
       ORDER BY student_id, updated_at DESC`);
        // 4. Map together
        const mapped = students.map((s) => {
            const att = attendanceRes.rows.find((a) => a.student_id === s.adm_no) || { total_classes: 0, present: 0, absent: 0, late: 0, leave: 0 };
            const hifz = hifzRes.rows.find((h) => h.student_id === s.adm_no);
            return {
                ...s,
                attendance: att,
...
const getMentorReports = async (req, res) => {
    try {
        ...
        const lastDay = new Date(targetYear, targetMonth, 0).getDate();
        const monthEnd = `${targetYear}-${String(targetMonth).padStart(2, '0')}-${String(lastDay).padStart(2, '0')}`;
        const mentorsRes = await db_1.db.query(`SELECT id, name, role, phone, is_active as active
        FROM staff 
        WHERE is_active = true
        ORDER BY name ASC`);
        const mentors = mentorsRes.rows;
        // Mentor attendance from staff_attendance
        const attRes = await db_1.db.query(`SELECT 
          staff_id,
        ...
        WHERE date >= $1 AND date <= $2
        GROUP BY staff_id`, [monthStart, monthEnd]);
        const mapped = mentors.map((m) => {
            const att = attRes.rows.find((a) => a.staff_id === m.id) || { total_marked: 0, present: 0, absent: 0, leave: 0 };
            return {
                ...m,
                attendance: att
            };
