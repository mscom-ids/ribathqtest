...
const enforceHifzStudentAccess = async (req, requests) => {
    const user = req.user;
    ...
    const delegation = await (0, staff_utils_1.getDelegationContext)(req);
    if (!delegation?.staffId) {
        const err = new Error('Mentor staff profile not found.');
        err.statusCode = 403;
        throw err;
    }
    if (delegation.studentId && requestedStudentIds.some((studentId) => studentId !== delegation.studentId)) {
        const err = new Error('This delegation does not allow Hifz entries for the selected student.');
        err.statusCode = 403;
        throw err;
    }
    ...
    const result = await db_1.db.query(`WITH requested AS (
             SELECT *
    ...
    const authorizedRequests = new Set(result.rows.map((row) => (`${String(row.student_id)}|${row.entry_date || ''}|${row.session_id || ''}`)));
    const hasUnauthorizedRequest = normalizedRequests.some((request) => (!authorizedRequests.has(`${request.studentId}|${request.entryDate || ''}|${request.sessionId || ''}`)));
    if (hasUnauthorizedRequest) {
        const err = new Error("Another mentor can record this student's Hifz progress only after the assigned mentor marks the student PRESENT.");
        err.statusCode = 403;
        throw err;
    }
...
const getDetectedLogDays = async (startDate, endDate) => {
    const result = await db_1.db.query(`SELECT COUNT(DISTINCT entry_date::date) AS log_days
         ...
         WHERE entry_date >= $1::date
           ...
           AND deleted_at IS NULL`, [startDate, endDate]);
...
const getHifzLog = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await db_1.db.query('SELECT * FROM hifz_logs WHERE id = $1 AND deleted_at IS NULL', [id]);
...
const getMaxJuzForStudent = async (req, res) => {
    try {
        ...
        const result = await db_1.db.query('SELECT juz_number FROM hifz_logs WHERE student_id = $1 AND deleted_at IS NULL ORDER BY juz_number DESC LIMIT 1', [student_id]);
...
const bulkCreateHifzLogs = async (req, res) => {
    try {
        ...
        await enforceHifzStudentAccess(req, logs.map((log) => ({
        ...
        const dupKey = (l) => `${l.student_id}|${toDateKey(l.entry_date)}|${l.mode}|${l.surah_name}|${l.start_v}|${l.end_v}`;
        const existingKeys = new Set();
        if (dedupCandidates.length > 0) {
            const studentIds = [...new Set(dedupCandidates.map((l) => l.student_id))];
            const dates = [...new Set(dedupCandidates.map((l) => toDateKey(l.entry_date)))];
            const existing = await db_1.db.query(`SELECT student_id,
                 ...
                 WHERE mode = ANY($3::text[])
                   ...
                   AND deleted_at IS NULL`, [studentIds, dates, ['New Verses', 'Recent Revision']]);
...
const deleteHifzLog = async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await db_1.db.query('SELECT student_id, entry_date, mode FROM hifz_logs WHERE id = $1 AND deleted_at IS NULL', [id]);
        if (existing.rows[0]) {
            const { student_id, entry_date, mode } = existing.rows[0];
            await enforceHifzRecordingAccess(req, entry_date);
            await enforceHifzStudentAccess(req, [{ studentId: student_id }]);
            await db_1.db.query(`DELETE FROM hifz_logs
                 WHERE student_id = $1
                   AND entry_date = $2
                   AND mode = $3`, [student_id, entry_date, mode]);
        }
        (0, server_cache_1.invalidateCacheByPrefix)('hifz:');
        (0, server_cache_1.invalidateCacheByPrefix)('reports:students');
    ...
    try {
        ...
        const progressMap = await (0, server_cache_1.cachedResult)((0, server_cache_1.makeCacheKey)('hifz:progress-summary', { student_id: student_id || 'all' }), HIFZ_SUMMARY_TTL_MS, async () => {
            ...
            const result = await db_1.db.query(`SELECT hl.student_id, hl.surah_name, hl.start_v, hl.end_v
                     ...
                     ${where}
                       AND hl.deleted_at IS NULL`, params);
            const byStudent = {};
        ...
        const [studentResult, logsResult] = await Promise.all([
            ...
            db_1.db.query(`SELECT mode, entry_date, surah_name, start_v, end_v, start_page, end_page, juz_portion
                 ...
                 WHERE student_id = $1 AND entry_date >= $2 AND entry_date <= $3
                   AND deleted_at IS NULL`, [student_id, startDate, endDate]),
        ]);
...
const monthlyRegisterError = (message, statusCode = 400) => {
    const error = new Error(message);
    error.statusCode = statusCode;
    return error;
};
const getMonthlyRegister = async (req, studentId, month) => {
    const academicContext = await (0, academic_year_1.getAcademicYearContext)(db_1.db, req.query.academic_year_id);
    return (0, hifz_monthly_register_service_1.getHifzStudentMonthRegister)({
        db: db_1.db,
        studentId,
        month,
...
const getRegisterMonth = (entryDate) => toDateKey(entryDate).slice(0, 7);
const getHifzStudentMonth = async (req, res) => {
    try {
...
exports.getHifzStudentMonth = getHifzStudentMonth;
const saveMonthlyHifzEntry = async (req, res, existingId) => {
    ...
    try {
        ...
        if (existingId && !existing)
            throw monthlyRegisterError('Hifz entry not found.', 404);
        ...
        const result = existing
            ? await client.query(`UPDATE hifz_logs
            ...
            : await client.query(`INSERT INTO hifz_logs (
        ...
        res.json({
            success: true,
            entry: result.rows[0],
            day: monthRegister.days.find((day) => day.date === toDateKey(log.entry_date)) || null,
 ...
 */
const batchSaveMonthlyHifzEntries = async (req, res) => {
    let client = null;
    ...
    try {
        ...
        const creates = (rawCreates || []).map((item, index) => normalizeHifzLogInput({
        ...
        const updates = (rawUpdates || []).map((item, index) => {
            ...
            if (!UUID_PATTERN.test(id))
                throw monthlyRegisterError(`Update ${index + 1}: invalid entry id.`);
            return {
                id,
        ...
        const deleteIds = (rawDeleteIds || []).map((value, index) => {
            const id = String(value || '').trim();
            if (!UUID_PATTERN.test(id))
                throw monthlyRegisterError(`Delete ${index + 1}: invalid entry id.`);
            return id;
        });
        const operationCount = creates.length + updates.length + deleteIds.length;
        if (operationCount === 0)
            throw monthlyRegisterError('At least one Hifz entry change is required.');
        ...
        const referencedIds = [...updates.map((item) => item.id), ...deleteIds];
        ...
        const [staffId, academicContext] = await Promise.all([
            ...
            (0, academic_year_1.getAcademicYearContext)(db_1.db, req.query.academic_year_id),
        ]);
        client = await db_1.db.getClient();
        await client.query('BEGIN');
        ...
        if (hasWrites) {
            const fallbackExisting = existingRows.find((row) => row.session_id);
            const eligibility = await (0, hifz_monthly_register_service_1.resolveHifzEntryEligibility)({
                db: client,
                studentId,
                entryDate,
        ...
        const deletedRows = deleteIds.length > 0
            ? (await client.query(`DELETE FROM hifz_logs
                 WHERE id = ANY($1::uuid[])
                 RETURNING id`, [deleteIds])).rows
            : [];
        let updatedRows = [];
        if (updates.length > 0) {
            const values = [];
            ...
            for (const item of updates) {
                ...
                values.push(item.id, entryDate, mode, item.log.surah_name, item.log.start_v, item.log.end_v, item.log.start_page, item.log.end_page, item.log.juz_number, item.log.juz_portion, effectiveSessionId);
            }
            const result = await client.query(`UPDATE hifz_logs AS target
        ...
        if (creates.length > 0) {
            ...
            for (const log of creates) {
                tuples.push(`($${parameter++},$${parameter++},$${parameter++},$${parameter++},$${parameter++},$${parameter++},$${parameter++},$${parameter++},$${parameter++},$${parameter++},$${parameter++},$${parameter++},`
                    ...
                    + `$${parameter++},$${parameter++})`);
                values.push(studentId, staffId || log.usthad_id || null, entryDate, mode, log.surah_name, log.start_v, log.end_v, log.start_page, log.end_page, log.juz_number, log.juz_portion, effectiveSessionId, staffId || null, log.notes);
            }
            const result = await client.query(`INSERT INTO hifz_logs (
                    student_id, usthad_id, entry_date, mode, surah_name, start_v, end_v,
                    start_page, end_page, juz_number, juz_portion, session_id,
                    attendance_record_id, created_by, notes
                 ) VALUES ${tuples.join(',')}
        ...
        return res.json({
            ...
            deleted_ids: deletedRows.map((row) => String(row.id)),
            day: monthRegister?.days?.find((day) => day.date === entryDate) || null,
...
exports.batchSaveMonthlyHifzEntries = batchSaveMonthlyHifzEntries;
const createMonthlyHifzEntry = async (req, res) => {
    await saveMonthlyHifzEntry(req, res);
};
exports.createMonthlyHifzEntry = createMonthlyHifzEntry;
const updateMonthlyHifzEntry = async (req, res) => {
    await saveMonthlyHifzEntry(req, res, String(req.params.id || ''));
};
exports.updateMonthlyHifzEntry = updateMonthlyHifzEntry;
const deleteMonthlyHifzEntry = async (req, res) => {
    try {
        const id = String(req.params.id || '');
        const existing = await db_1.db.query('SELECT id, student_id, entry_date FROM hifz_logs WHERE id = $1 AND deleted_at IS NULL', [id]);
...
exports.deleteMonthlyHifzEntry = deleteMonthlyHifzEntry;
