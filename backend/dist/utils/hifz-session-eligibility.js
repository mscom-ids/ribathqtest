...
exports.normalizeHifzStandard = normalizeHifzStandard;
exports.parseHifzStandardList = parseHifzStandardList;
exports.isHifzSchedule = isHifzSchedule;
exports.findHifzSessionForSchedule = findHifzSessionForSchedule;
exports.resolveHifzStandardsForSchedule = resolveHifzStandardsForSchedule;
exports.getEligibleHifzStudentsForSchedule = getEligibleHifzStudentsForSchedule;
const db_1 = require("../config/db");
const HIFZ_SESSION_CACHE_TTL_MS = 5 * 60000;
const hifzSessionCache = new Map();
const hifzRuleStandardCache = new Map();
const hifzSessionInFlight = new Map();
const hifzRuleStandardInFlight = new Map();
function normalizeHifzStandard(label) {
...
function parseHifzStandardList(value) {
...
function isHifzSchedule(schedule) {
    return String(schedule?.class_type || '').toLowerCase() === 'hifz';
}
async function getActiveHifzSessions(academicYearId) {
    ...
    const request = db_1.db.query(`SELECT id, name, code
         FROM hifz_sessions
         WHERE academic_year_id = $1
           AND is_active = true
         ORDER BY sort_order, name`, [academicYearId]).then((result) => {
...
async function getRuleStandards(academicYearId, hifzSessionId, date) {
    ...
    if (existingRequest)
        return existingRequest;
    const request = db_1.db.query(`SELECT DISTINCT standard
         FROM hifz_session_rules
         WHERE academic_year_id = $1
           AND hifz_session_id = $2
           AND is_active = true
           AND ${dateWindowSql('hifz_session_rules')}
         ORDER BY standard`, [academicYearId, hifzSessionId, date || null]).then((result) => {
...
async function findHifzSessionForSchedule(schedule, academicYearId) {
...
async function resolveHifzStandardsForSchedule(schedule, academicYearId, date) {
    ...
    const standards = rows.map((row) => String(row.standard || '').trim()).filter(Boolean);
    if (standards.length === 0)
        return { standards: fallbackStandards, usedRules: false, session };
    return { standards, usedRules: true, session };
}
async function getEligibleHifzStudentsForSchedule(options) {
    const { schedule, academicYearId, date, mentorId } = options;
    const fallbackStandards = parseHifzStandardList(schedule?.standards).map(normalizeHifzStandard).filter(Boolean);
    if (!academicYearId || !isHifzSchedule(schedule)) {
        return { usedRules: false, students: null, standards: fallbackStandards, session: null };
    }
    const session = await findHifzSessionForSchedule(schedule, academicYearId);
    if (!session)
        return { usedRules: false, students: null, standards: fallbackStandards, session: null };
    const rulesRes = await db_1.db.query(`SELECT COUNT(*)::integer AS total
         FROM hifz_session_rules
         WHERE academic_year_id = $1
           AND hifz_session_id = $2
           AND is_active = true
           AND ${dateWindowSql('hifz_session_rules')}`, [academicYearId, session.id, date || null]);
    if (Number(rulesRes.rows[0]?.total || 0) === 0) {
        return { usedRules: false, students: null, standards: fallbackStandards, session };
    }
    const params = [academicYearId, session.id, date || null];
    let mentorFilter = '';
    if (mentorId) {
        params.push(mentorId);
        mentorFilter = `AND eligible.hifz_mentor_id = $${params.length}`;
    }
    const result = await db_1.db.query(`WITH base_students AS (
        ...
        ${mentorFilter}
        ORDER BY standard, name`, params);
    const standards = Array.from(new Set(result.rows.map((row) => row.standard).filter(Boolean)));
