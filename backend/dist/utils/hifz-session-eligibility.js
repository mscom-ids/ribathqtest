...
async function findHifzSessionForSchedule(schedule, academicYearId) {
    if (!academicYearId || !isHifzSchedule(schedule))
        return null;
    const result = await db_1.db.query(`SELECT id, name, code
         FROM hifz_sessions
    ...
    if (!scheduleName)
        return null;
    return result.rows.find((session) => {
...
async function resolveHifzStandardsForSchedule(schedule, academicYearId, date) {
    ...
    if (!session)
        return { standards: fallbackStandards, usedRules: false, session: null };
    const result = await db_1.db.query(`SELECT DISTINCT standard
         FROM hifz_session_rules
         WHERE academic_year_id = $1
           AND hifz_session_id = $2
           AND is_active = true
           AND ${dateWindowSql('hifz_session_rules')}
         ORDER BY standard`, [academicYearId, session.id, date || null]);
    const standards = result.rows.map((row) => String(row.standard || '').trim()).filter(Boolean);
